    ...
    public static function paginate( $args=array() )
    {
        ...
        if ( $args[ 'first_and_last' ] && ( $page_cur > 1 ) ) {
            $html .= sprintf(
                ...
                get_pagenum_link(),
                $args[ 'first_label' ],
                ( $args[ 'first_class' ] ) ? ' class="' . $args[ 'first_class' ] . '"' : ''
            );
            $html .= sprintf(
                '<a href="%1$s" title="%2$s"%3$s>%2$s</a>',
                get_pagenum_link( $page_cur-1 ),
                $args[ 'prev_label' ],
                ( $args[ 'prev_class' ] ) ? ' class="' . $args[ 'prev_class' ] . '"' : ''
            );
        }
        ...
        if ( $args[ 'first_and_last' ] && ( $page_cur < $page_tot ) ) {
            $html .= sprintf(
                ...
                get_pagenum_link( $page_cur+1 ),
                $args[ 'next_label' ],
                ( $args[ 'next_class' ] ) ? ' class="' . $args[ 'next_class' ] . '"' : ''
            );
            $html .= sprintf(
                '<a href="%1$s" title="%2$s"%3$s>%2$s</a>',
                get_pagenum_link( $page_tot ),
                $args[ 'last_label' ],
                ( $args[ 'last_class' ] ) ? ' class="' . $args[ 'last_class' ] . '"' : ''
            );
        }
