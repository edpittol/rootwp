...
class Option extends Dashboard
{
    ...
    public static function add_screen( $screen_id, $role, $args=array() )
    {
        ...
        self::$screens[ $screen_id ] = wp_parse_args( $args, $screen_opt );
    }
    ...
    public static function edit()
    {
        ...
        if ( isset( self::$screens[ $screen ] ) ) {
            ...
            if ( isset( $s[ 'callback' ] ) ) {
                if ( is_string( $s[ 'callback' ] ) && function_exists( $s[ 'callback' ] ) )
                    call_user_func( $s[ 'callback' ] );

                list( $class, $method ) = ( is_array( $s[ 'callback' ] ) ) ? $s[ 'callback' ] : array( false, false );
