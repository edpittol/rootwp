...
class Metabox extends Dashboard
{
    ...
    public static function edit( $post, $metabox )
    {
        ...
        echo '<input type="hidden" name="root_metabox" value="1" />';
    ...
    public static function save( $post_id )
    {
        ...
        if (
            ...
            do_action( 'metabox_save', $args[ 'object_type' ], $args[ 'object_id' ], $post_type );
            $args = apply_filters( 'metabox_save_args', $args, $post_type );
            if ( is_array( $args ) )
                self::meta_save( $args );
        }
    }
    ...
    private static function set_values( $post )
    {
        if ( !in_array( $post->post_status, array( 'auto-draft', 'trash', 'inherit' ) ) ) {
            ...
            $args = apply_filters( 'metabox_values_args', $args, $post->post_type );
            if ( is_array( $args ) ) {
