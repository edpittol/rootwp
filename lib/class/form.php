...
class Form
{
    ...
    private function is_attr( $type, $attr )
    {
        ...
        switch ( $type ) {
            case 'form':
                ...
                break;
            case 'text':
                array_push( $attrs , 'placeholder', 'size' );
                break;
            case 'textarea':
    ...
    private function select( $f )
    {
        if ( is_array( $f[ 'opt' ] ) ) {
            ...
            $html = sprintf(
                '<select%s>%s</select>',
                $this->print_attr( $f ),
                $this->select_options( $f[ 'opt' ], $value, $mult )
            );
        }
        else {
     ...
     *
     */
    private function select_options( $options, $values, $mult=false )
    {
        $html = ( $mult ) ? '' : sprintf( '<option value="">%s</option>', __r( 'Choose' ) );
    ...
    private function upload( $f )
    {
        ...
        $html .= sprintf(
            '<input type="button" class="button upload-button" value="%s" id="%s-upload" />',
            __r( 'Library' ),
            $f[ 'id' ]
        );
        return $html;
    ...
    private function render_html()
    {
        if ( is_array( $this->fields ) ) {
            ...
            foreach ( $this->fields as $f ) {
                ...
                if ( isset( $f[ 'desc' ] ) )
                    $html_field .= sprintf( '<p>%s</p>', $f[ 'desc' ] );
